-- Comisión técnica por orden de trabajo.
-- La configuración vive en la OT; la base y el monto se congelan al vender,
-- siempre usando exclusivamente la línea `ot-servicio` neta (sin repuestos).
...
declare
  ...
  v_antes numeric;
  v_despues numeric;
  v_comision_activa boolean := false;
  v_comision_porcentaje numeric := 0;
  v_comision_base numeric := 0;
  v_comision_monto numeric := 0;
begin
  ...
  if not v_permitir_sin_stock and p_ajustes_stock is not null then
    ...
    if v_faltantes is not null then raise exception 'Stock insuficiente: %', v_faltantes; end if;
  end if;
  ...
  if p_orden is not null then
    v_comision_activa := coalesce((p_orden->>'comision_tecnica_activa')::boolean, false);
    v_comision_porcentaje := greatest(0, least(100, coalesce((p_orden->>'comision_tecnica_porcentaje')::numeric, 0)));
    if v_comision_activa and v_comision_porcentaje > 0 then
      select coalesce(sum((i->>'subtotal')::numeric), 0)
        into v_comision_base
      from jsonb_array_elements(p_items) i
      where i->>'producto_id' = 'ot-servicio';
      v_comision_monto := round(v_comision_base * v_comision_porcentaje / 100);
    end if;
    ...
    set status = p_orden->>'status', venta_id = p_orden->>'venta_id',
        ...
        comision_tecnica_base = case when v_comision_activa then v_comision_base else null end,
        comision_tecnica_monto = case when v_comision_activa then v_comision_monto else null end
