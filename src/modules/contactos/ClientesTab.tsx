...
function DetailPanel({ cliente, stats, esAdmin, onEditar, onEliminar }: {
  cliente: Cliente
  stats: { ots: ReturnType<typeof useOrdenes>['data'] extends (infer T)[] | undefined ? T[] : never; boletas: ReturnType<typeof useVentas>['data'] extends (infer T)[] | undefined ? T[] : never; totalVentas: number } | null
  esAdmin: boolean
  onEditar: () => void
  onEliminar: () => void
}) {
    ...
    <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', overflowY: 'auto', background: '#fff' }}>
      ...
      <div style={{ padding: '20px 24px', flex: 1 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10, marginBottom: 24 }}>
          {[
            { val: `$${Math.round(stats?.totalVentas ?? 0).toLocaleString('es-CL')}`, lbl: 'Total comprado' },
            { val: String(stats?.boletas.length ?? 0), lbl: 'Boletas' },
            { val: String(otsActivas.length), lbl: 'OTs activas' },
          ].map(s => (
            <div key={s.lbl} style={{ background: '#f9fafb', border: '0.5px solid #e5e7eb', borderRadius: 10, padding: '10px 14px' }}>
              <p style={{ margin: '0 0 2px', fontSize: 15, fontWeight: 600, color: '#111' }}>{s.val}</p>
              <p style={{ margin: 0, fontSize: 11, color: '#9ca3af' }}>{s.lbl}</p>
            </div>
          ))}
        </div>
