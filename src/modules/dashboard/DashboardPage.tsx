...
export function DashboardPage() {
  ...
  const stats = useMemo(() => {
    ...
    const branchStats = bodegas.map((b, i) => {
      ...
      const bTicket = bOTcomp.length ? Math.round(bOTcomp.reduce((s, o) => s + (+o.total || 0), 0) / bOTcomp.length) : 0
      return {
    ...
    const totalOTticket = otsMes.length ? Math.round(otsMes.reduce((s, o) => s + (+o.total || 0), 0) / otsMes.length) : 0
