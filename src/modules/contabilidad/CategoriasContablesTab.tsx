...
export function CategoriasContablesTab() {
  ...
  // Al Debe de un gasto no siempre va una cuenta de gasto, y filtrar por eso
  // dejaba fuera casos corrientes:
  //   - Pagar cotizaciones ya devengadas → Debe un PASIVO (lo disminuye).
  //   - Comprar una herramienta → Debe un ACTIVO, no un gasto.
  //   - Retiro del dueño → Debe PATRIMONIO.
  // Se excluyen los ingresos: debitar una cuenta de ingreso desde la pantalla
  // de gastos casi siempre es un error de selección, no una intención.
  const GRUPOS: { tipo: string; titulo: string }[] = [
    { tipo: 'gasto', titulo: 'Gastos' },
    { tipo: 'pasivo', titulo: 'Pasivos — pago de obligaciones ya registradas' },
    { tipo: 'activo', titulo: 'Activos — compras que quedan en el patrimonio' },
    { tipo: 'patrimonio', titulo: 'Patrimonio — retiros y aportes' },
  ]
  const cuentasPorGrupo = GRUPOS
    .map(g => ({ ...g, cuentas: (planCuentas ?? []).filter(c => c.tipo === g.tipo) }))
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
      ...
      <div className="divide-y divide-gray-200">
        {(cats ?? []).map(cat => {
          const cuentaId = catCuentaMap?.[cat.nombre] ?? 'pc-595'
          const cuenta = (planCuentas ?? []).find(c => c.id === cuentaId)
          // Pixit no registra devengos: el costo entra solo al pagarlo. Si el
          // Debe apunta a un pasivo o un activo, ese monto NO aparece en el
          // Estado de Resultados, y la utilidad queda más alta que la real.
            ...
            <select
              ...
              {cuentasPorGrupo.map(g => (
                <optgroup key={g.tipo} label={g.titulo}>
                  {g.cuentas.map(c => <option key={c.id} value={c.id}>{c.codigo} — {c.nombre}</option>)}
                </optgroup>
              ))}
            </select>
      ...
      <div className="px-4 py-3 bg-gray-50 text-xs text-gray-500 flex items-center gap-2">
        ...
        La cuenta elegida va al Debe del asiento. Además de gastos podés usar un pasivo (pagar algo ya registrado), un activo (una compra que conservás) o patrimonio (retiros). Los cambios se guardan al instante.
      </div>
